...
import { Building } from 'ggtu-timetable-api-client';
...
export default class Cabinets extends entityListPage(cabinets, CabinetsLoader) {
  ...
  filter(state: EntitiesState) {
    return (id: number) => {
      const cabinet = state.entities[id] as Cabinet;
      const building = cabinet.building as Building;
      const name = `${building.name.split(' ')[1]}к${cabinet.number}`;
      return name.toLowerCase().includes(state.filterValue);
    };
  }
}
</script>
